...
<body>
	...
	<c:if test="${member != null}">
		<form action="QnAWriteGO" class="writeBtn">
	...
	<table>
		...
		<c:forEach var="qna" items="${qnaList.qnaList}">
				<tr align="center">
					...
					<td width="100">${qna.kind}</td>
					<c:if test="${qna.commentNumber == '0'}">
					<td width="500" style="font-size: 11px;"><a href="QnAContentGO?idx=${qna.idx}">${qna.title}</a></td>
					</c:if>
					<c:if test="${qna.commentNumber != '0'}">
					<td width="500" style="font-size: 11px;"><a href="QnAContentGO?idx=${qna.idx}">${qna.title}(${qna.commentNumber})</a></td>
					</c:if>
